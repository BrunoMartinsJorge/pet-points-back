package br.com.api.petpoints.modules.users.cliente.features.meuperfil.dto;

import br.com.api.petpoints.shared.models.UsuarioModel;
import br.com.api.petpoints.shared.utils.LocalDateTimeUtils;
import br.com.api.petpoints.shared.utils.LocalDateUtils;
import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;

@Getter
@Setter
@AllArgsConstructor
@NoArgsConstructor
public class MeuPerfilDto {

    private Long id;
    private String email;
    private String nome;
    private String cpf;
    private String telefone;
    private String genero;
    private String dataNascimento;
    private String dataCadastro;
    private String status;

    public MeuPerfilDto(UsuarioModel usuario) {
        this.id = usuario.getId();
        this.email = usuario.getEmail();
        this.nome = usuario.getNome();
        this.cpf = usuario.getCpf();
        this.telefone = usuario.getTelefone();
        this.genero = usuario.getGenero().getDescricao();
        this.dataNascimento = LocalDateUtils.converterLocalDateParaPtBr(usuario.getDataNascimento());
        this.dataCadastro = LocalDateTimeUtils.converterLocalDateTimeParaPtBr(usuario.getDataCadastro());
        this.status = usuario.getStatusPerfilEnum().getDescricao();
    }
}
